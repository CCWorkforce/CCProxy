...
class TestDispatcherPerformance:
    ...
    def test_dispatcher_multiple_blocks(self, benchmark):
        ...
        dispatcher.register_image_handler(image_handler)
        dispatcher.register_tool_use_handler(tool_use_handler)
        dispatcher.register_tool_result_handler(tool_result_handler)

        def dispatch_all():
            blocks = [TEXT_BLOCK, IMAGE_BLOCK, TOOL_USE_BLOCK, TOOL_RESULT_BLOCK]
            return [dispatcher.dispatch(block) for block in blocks]
